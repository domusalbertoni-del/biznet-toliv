        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
          {events.map((event) => (
            <a
              key={event.id}
              href="#"
              className="group block rounded-2xl overflow-hidden bg-card hover:glow-primary transition-all duration-300"
            >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex gap-2">
                    {event.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2.5 py-1 rounded-full bg-secondary text-[11px] text-muted-foreground font-medium"
                      >
                        {tag}
                      </span>
                    ))}
